...
class AntFeatureGraph:    
    ...
    As the edges are lazily generated there is no efficient way to quickly get the neighbours as storage may exceed memory 
    ...
    pheromone_initial : array-like of shape (n_nodes,)
        Contains the SU for each edge (initial node, node_i)

    pheromone_selection : dict
    ...
    def _initialize_initial_matrix(self,X,y):
        '''Initializes the initial_heuristic and the pheromone_initial'''
        self.initial_heuristic = np.empty(len(self.nodes))
        self.pheromone_initial = np.random.rand(len(self.nodes))
    ...
    def get_neighbours(self, node, nodes_to_filter, step):
        ...
        if step == "CONSTRUCTION":
            ...
            for neighbour_id, values in filter(lambda x: x[1][1] is not None  and x[0]!=node_id, self.nodes.items()):
                for operator in (self.operators if values[0]!=feature else ['OR']):
                    ...
                    if edge in nodes_to_filter:
                        continue
                    if edge not in self.pheromone_construction:
            ...
            for neighbour_id, values in filter(lambda x: x[0] not in nodes_to_filter and not(x[0] == node_id and value is None), self.nodes.items()):
                ...
                if edge not in self.pheromone_selection:
    ...
    def get_initial_nodes(self,selected_nodes):
        ...
        nodes_pheromone_heuristic = zip(sorted(list(self.nodes.items()),key=lambda x: x[0]), self.pheromone_initial, self.initial_heuristic) 
    ...
    def update_pheromone_matrix_evaporation(self, evaporation_rate):
        ...
        self.pheromone_initial =  self.pheromone_initial*update_factor
...
class AntFeatureGraphMI:
    ...
    Pruned graph containing the edges for the ants to explore and the pheromone matrix.
    In this version the graph is eagerly generated.
...
class AntFeatureGraphMILazy:
    """ Graph for the ACFCS algorithm.
    
    Graph is pruned and lazility generated
    The prunning is based on symmetrical uncertainty
    Parameters
    ----------
    seed : float or None
        Seed used for the generation of random values for the pheromone trail.
    ...
    def _initialize_initial_matrix(self,X,y):
        ...
        for node, value in self.nodes.items():
            ...
            self.initial_heuristic[node] = su
            self.pheromone_initial[node] = random.random()
