...
class CustomOrdinalFeatureEncoder(TransformerMixin, BaseEstimator):
    ...
    discretizer : KBinsDiscretizer -> Strategy K-means
    ...
    def fit(self, X, y=None):
        ...
        if X.dtype == "O":
            '''Mixed types have strange behaviours with searchsorted'''
            X = X.astype(str)
        ...
        self.sorted_encoded_ = [np.arange(self.categories_[j].shape[0])[self.sort_index_[j]]
                    ...
                    if index:
                        ...
                        for i in sort_index:
                            ...
                            self.discretizer.n_bins_ = np.insert(
                                self.discretizer.n_bins_, last, temp_discretizer.n_bins_[i], axis=1)
                            self.discretizer.bin_edges_ = np.insert(
                                self.discretizer.n_bins_, last, temp_discretizer.bin_edges_[i], axis=1)
                        ...
                        self.discretizer.n_bins_ = np.concatenate(
                            [self.discretizer.n_bins_, temp_discretizer.n_bins_], axis=1)
                        self.discretizer.bin_edges_ = np.concatenate(
                            [self.discretizer.bin_edges_, temp_discretizer.bin_edges_], axis=1)
            X = X.to_numpy()
    ...
    def remove_feature(self, index):
        ...
        if self.discretize:
            try:
                ...
                # Not a continuous feature
                pass

    def inverse_transform_element(self, feature_index, value):
        '''Used to translate a single value of a given feature. Used for translation'''
        check_is_fitted(self)
        try:
