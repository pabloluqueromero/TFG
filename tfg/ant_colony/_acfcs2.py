...
class ACFCS2(TransformerMixin,ClassifierMixin,BaseEstimator):
    ...
    def fit(self,X,y,init_graph=True):
        ...
        for iteration in iterator:
            ...
            ants = [Ant2(ant_id=i,alpha=self.alpha,beta=beta, metric = self.metric, use_initials = self.use_initials, cache_loo = self.cache_loo, cache_heuristic = self.cache_heuristic,step = self.step) for i in range(self.ants)]
            beta*=self.beta_evaporation_rate
            results = []
            for ant in ants:
                results.append(ant.run(X=X,y=y,graph=self.afg,random_generator=random,parallel=self.parallel,max_errors = self.max_errors, initial_attributes= next_features))
            ...
            next_features = [feature.copy() for feature in ant.current_features]
